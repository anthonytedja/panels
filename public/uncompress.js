...
(function () {
  ...
  function archiveOpenArrayBuffer(file_name, password, array_buffer) {
    ...
    if (_loaded_archive_formats.indexOf(archive_type) === -1) {
      throw new Error(
        "The archive format '" + archive_type + "' is not loaded."
      );
    }
  ...
  function _zipGetEntries(zip_handle) {
    ...
    Object.keys(zip.files).forEach(function (i) {
      ...
      let size_compressed = zip_entry._data
        ? zip_entry._data.compressedSize
        : 0;
      let size_uncompressed = zip_entry._data
  ...
  function isRarFile(array_buffer) {
    ...
    let rar_header2 = saneJoin(
      [0x52, 0x61, 0x72, 0x21, 0x1a, 0x07, 0x00],
      ", "
    ); // 1.5 to 4.0
    let rar_header3 = saneJoin(
